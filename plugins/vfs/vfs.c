...
static size_t
find_parent(struct container *c, const char *path, size_t len)
{
   assert(c && path && len);
   ...
   size_t index = NONODE;
   for (char *s = strchr(tmp.data, '/'); s && *s; s = strchr(tmp.data, '/')) {
      *s = 0;
      if ((index = find_node(c, s)) == NONODE)
         goto out;
   }
...
add_node(const char *container, const char *path, const char *type, void *arg, const struct function *read, const struct function *write, const struct function *clunk, const struct function *size)
{
   ...
   size_t parent = (name - path > 0 ? find_parent(c, path, name - path) : c->root);
   if (parent == NONODE)
      return false;
